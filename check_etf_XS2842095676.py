...
def check_etf_XS2842095676():
    """Check specific ETF XS2842095676 for leveraged keywords"""
    ...
    try:
        print("🔍 Checking XS2842095676...")
        
        # Get the specific ETF
        response = supabase.table('etf_funds').select('*').eq('isin', 'XS2842095676').execute()
        ...
        print(f"🎯 Currently marked as leveraged: {etf['is_leveraged']}")
        
        if should_be_leveraged and not etf['is_leveraged']:
            print(f"\n🔄 This ETF should be marked as leveraged but isn't!")
            print(f"   Updating now...")
            
            update_response = supabase.table('etf_funds').update({
                'is_leveraged': True
            }).eq('isin', 'XS2842095676').execute()
            
            if update_response.data:
                print(f"   ✅ Successfully updated!")
            else:
                print(f"   ❌ Update failed!")
        elif not should_be_leveraged and etf['is_leveraged']:
            print(f"\n❓ This ETF is marked as leveraged but doesn't match keywords")
            print(f"   Should we remove the leveraged flag? (might be manually set)")
        elif should_be_leveraged and etf['is_leveraged']:
            print(f"\n✅ ETF is correctly marked as leveraged")
        else:
            print(f"\n✅ ETF is correctly marked as non-leveraged")
        
    except Exception as e:
        print(f"❌ Error: {e}")

if __name__ == "__main__":
    check_etf_XS2842095676()
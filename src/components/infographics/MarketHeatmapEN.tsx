...
const MarketHeatmapEN: React.FC<MarketHeatmapProps> = ({ data }) => {
  ...
  const renderHeatmapGrid = (categoryData: { [key: string]: MarketData }, title: string, categoryType: 'sectors' | 'regions' | 'assets') => {
    ...
    return (
      <div className="mb-12 bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
        {/* Header for each section - English version */}
        <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 text-white p-4">
          <div className="text-center">
            ...
            <div className="text-xs text-purple-200 mt-1">
              📊 {new Date(data.metadata.generated_at).toLocaleDateString('en-US')} | {data.metadata.data_source}
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className={`grid gap-3 ${
            categoryType === 'sectors' 
              ? 'grid-cols-4 lg:grid-cols-4 xl:grid-cols-4' // 3 rows for sectors
              : categoryType === 'regions'
                ? 'grid-cols-3 lg:grid-cols-3 xl:grid-cols-3' // 3 columns for regions
                : 'grid-cols-3 lg:grid-cols-4 xl:grid-cols-4' // 3-4 columns for assets
          }`}>
            {items.map(([name, data]) => {
          ...
          {/* Footer with English branding */}
          <div className="mt-4 pt-3 border-t border-gray-200">
            <div className="flex items-center justify-between">
              <div className="text-xs text-gray-600">
                💡 Data from representative ETF funds
              </div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8">
      {/* Heatmap grids - each with own header for screenshots */}
      {renderHeatmapGrid(data.sectors, '📈 S&P 500 Sectors', 'sectors')}
      {renderHeatmapGrid(data.regions, '🌍 Regions & Countries', 'regions')}
      {renderHeatmapGrid(data.asset_classes, '💰 Asset Classes', 'assets')}
    </div>
  );
};

export default MarketHeatmapEN;